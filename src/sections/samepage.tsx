...
export const SamePage = () => {
    ...
    // we are declaring main parts of the animation - the referenced component (aka targetRef) and the offset value for each part of it
    // then we are declaring values of the basic animations, scale, opacity, x, y, etc. 
    // and then we declaring each details component animation and referencing then as object properties to the main animation(opacity, sx, y, etc.)
    // hense this gives the animations a higher order and some structure to the code
    ...
    //The animationOrder object is a mapping of keyframes for your animation. Each property represents a keyframe, and the value is the scroll position at which that keyframe should occur. The scroll position is expressed as a percentage of the viewport height. For example, fadeInEnd: 0.15 means that the fadeInEnd keyframe should occur when the user has scrolled 15% of the viewport height.

    //The useTransform function from Framer Motion is used to create animations based on the scroll position. It takes three arguments:

    //The first argument is the source of the transformation. In this case, it's scrollYProgress, which is a MotionValue that represents the current scroll position as a fraction of the viewport height.

    //The second argument is an array of input range values. These values correspond to the scroll positions at which the animation should change. They are taken from the animationOrder object.

    //The third argument is an array of output range values. These values correspond to the animation values at each keyframe. For example, in the opacity animation, the output range is [0, 1, 1, 0], which means the opacity should start at 0, increase to 1 at the fadeInEnd keyframe, stay at 1 until the createBranchEnd keyframe, and then decrease back to 0 at the endTextFadeInStart keyframe.

    //The pos variable in the position and avatarOpacity animations is the current scroll position. It's used to determine whether the position should be relative or fixed, and whether the avatar should be visible or not, based on the scroll position.
